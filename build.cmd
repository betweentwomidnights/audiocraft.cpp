@echo off
rem Build audiocraft.cpp for one backend into its own build dir (backends coexist for A/B).
rem
...
if /i "%BACKEND%"=="cpu" (
    ...
    set "DIR=build-cuda" & set "FLAGS=-DAC_CUDA=ON -DCMAKE_CUDA_ARCHITECTURES=native"
) else if /i "%BACKEND%"=="vulkan" (
    set "DIR=build-vulkan" & set "FLAGS=-DAC_VULKAN=ON"
    if "%VULKAN_SDK%"=="" echo [ac] WARNING: VULKAN_SDK not set - install the Vulkan SDK and open a fresh shell.
) else if /i "%BACKEND%"=="all" (
    set "DIR=build-all" & set "FLAGS=-DGGML_BACKEND_DL=ON -DGGML_CPU_ALL_VARIANTS=ON -DAC_CUDA=ON -DAC_VULKAN=ON"
) else (
...
cmake --build %DIR% --config Release --parallel || exit /b 1
echo [ac] built %BACKEND% -^> %DIR%\bin\Release\
