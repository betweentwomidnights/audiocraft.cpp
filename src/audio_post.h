// Ported from sa3.cpp src/audio_post.h (namespace sa3 -> ac, SA3_* env vars -> AC_*).
#pragma once
...
inline LoudnessParams loudness_defaults_from_env() {
    LoudnessParams p;
    p.latent_rescale = env_float("AC_LATENT_RESCALE", p.latent_rescale);
    p.latent_shift = env_float("AC_LATENT_SHIFT", p.latent_shift);
    env_optional_float("AC_LATENT_TARGET_STD", p.latent_target_std_enabled, p.latent_target_std);
    p.latent_adapt_min = env_float("AC_LATENT_ADAPT_MIN", p.latent_adapt_min);
    p.latent_adapt_max = env_float("AC_LATENT_ADAPT_MAX", p.latent_adapt_max);
    env_optional_float("AC_PEAK_NORMALIZE_DB", p.peak_normalize_enabled, p.peak_normalize_db);
    env_optional_float("AC_LIMITER_CEILING_DB", p.limiter_enabled, p.limiter_ceiling_db, true);
    p.limiter_knee = env_float("AC_LIMITER_KNEE", p.limiter_knee);
    return p;
}
...
inline SpliceParams splice_defaults_from_env() {
    SpliceParams p;
    p.enabled      = env_bool("AC_CONTINUE_SPLICE_SOURCE", p.enabled);
    p.mask_overlap = std::max(0.0f, env_float("AC_CONTINUE_MASK_OVERLAP", p.mask_overlap));
    p.xfade        = env_float("AC_CONTINUE_SPLICE_XFADE", p.xfade);
    p.gain_match   = env_bool("AC_CONTINUE_SPLICE_GAIN_MATCH", p.gain_match);
    return p;
}
