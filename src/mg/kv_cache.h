// mg/kv_cache.h -- the key/value history for one autoregressive stream.
//
...
//
//   K  [head_dim, capacity, n_head]   the layout attention wants for `mul_mat(k, q)`
...
// for nothing.
//
// One cache is one stream. Classifier-free guidance runs two streams over the same weights,
// so it holds two of these.
#pragma once
...
public:
    KvCache(ggml_backend_t backend, int layers, int head_dim, int n_head, int capacity)
        ...
        if (layers <= 0 || head_dim <= 0 || n_head <= 0 || capacity <= 0)
        ...
        for (int i = 0; i < layers; ++i) {
            k_[(size_t)i] = ggml_new_tensor_3d(ctx_, GGML_TYPE_F32, head_dim, capacity, n_head);
            v_[(size_t)i] = ggml_new_tensor_3d(ctx_, GGML_TYPE_F32, capacity, head_dim, n_head);
    ...
    size_t bytes() const {
        return (size_t)layers_ * 2 * head_dim_ * capacity_ * n_head_ * sizeof(float);
    }

    // Where this step's keys go: [head_dim, n_tokens, n_head] at column `used`.
    ggml_tensor* k_slot(ggml_context* ctx, int layer, int n_tokens) const {
        ggml_tensor* c = k_[(size_t)layer];
        return ggml_view_3d(ctx, c, head_dim_, n_tokens, n_head_, c->nb[1], c->nb[2],
                            (size_t)used_ * c->nb[1]);
    }
    // And its values, transposed: [n_tokens, head_dim, n_head] at row `used`.
    ggml_tensor* v_slot(ggml_context* ctx, int layer, int n_tokens) const {
        ggml_tensor* c = v_[(size_t)layer];
        return ggml_view_3d(ctx, c, n_tokens, head_dim_, n_head_, c->nb[1], c->nb[2],
                            (size_t)used_ * ggml_element_size(c));
    }

    // The whole history including this step: [head_dim, used + n_tokens, n_head].
    ggml_tensor* k_history(ggml_context* ctx, int layer, int n_tokens) const {
        ggml_tensor* c = k_[(size_t)layer];
        return ggml_view_3d(ctx, c, head_dim_, used_ + n_tokens, n_head_, c->nb[1], c->nb[2], 0);
    }
    // [used + n_tokens, head_dim, n_head], the layout nn::sdpa_vt consumes directly.
    ggml_tensor* v_history(ggml_context* ctx, int layer, int n_tokens) const {
        ggml_tensor* c = v_[(size_t)layer];
        return ggml_view_3d(ctx, c, used_ + n_tokens, head_dim_, n_head_, c->nb[1], c->nb[2], 0);
    }

private:
    int layers_ = 0, head_dim_ = 0, n_head_ = 0, capacity_ = 0, used_ = 0;
