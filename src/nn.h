// nn.h — stateless GGML neural-net building blocks shared across audiocraft.cpp.
// Ported from sa3.cpp src/nn.h (namespace sa3::nn -> ac::nn, SA3_* env vars -> AC_*).
...
inline bool flash_attn_enabled() {
    const char* p = getenv("AC_FLASH_ATTN");
    return p && strcmp(p, "0") != 0;
}

inline int same_flash_attn_mode() {
    const char* p = getenv("AC_SAME_FLASH_ATTN");
