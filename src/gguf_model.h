// Ported from sa3.cpp src/gguf_model.h (namespace sa3 -> ac, SA3_* env vars -> AC_*).
...
inline int cpu_threads_from_env() {
    const char* v = getenv("AC_THREADS");
    if (!v || !*v) return 0;
    char* end = nullptr;
    long n = strtol(v, &end, 10);
    if (!end || *end != '\0' || n <= 0 || n > 1024) {
        fprintf(stderr, "[ac] ignoring invalid AC_THREADS='%s' (expected a positive integer)\n", v);
        return 0;
    }
    return (int)n;
}
...
inline void configure_cpu_threads(ggml_backend_t b, int n_threads) {
    ...
    if (set_threads) {
        ...
        fprintf(stderr, "[ac] CPU threads: %d\n", n_threads);
    }
}

// Pick the compute backend: a registered GPU/iGPU device (CUDA when the CUDA backend is
// linked) unless AC_DEVICE=cpu forces CPU. In a CPU-only build the registry has no
...
// GPU/APU is present, use that instead of silently falling back to CPU. AC_GPU overrides this:
// a 0-based index into the GPU/iGPU list, or a case-insensitive substring of the device name
// (e.g. AC_GPU=nvidia). CUDA builds expose a single GPU device, so this is a no-op there.
...
inline ggml_backend_t make_backend(int cpu_threads = 0, const char* device = nullptr) {
        ...
        if (!gpus.empty()) {
            ...
            if (sel && *sel) {
                ...
                if (end && *end == '\0' && idx >= 0 && (size_t)idx < gpus.size()) {
                    ...
                    if (!chosen) fprintf(stderr, "[ac] AC_GPU='%s' matched no device; using default\n", sel);
                }
            }
            if (!chosen) {
            ...
            if (b) {
                ...
                fprintf(stderr, "[ac] backend: %s (%s, %.1f GB)\n",
    ...
    if (b) {
        ...
        if (d) {
            fprintf(stderr, "[ac] backend: %s (%s)\n",
                    ggml_backend_name(b), ggml_backend_dev_description(d));
        } else {
            fprintf(stderr, "[ac] backend: %s\n", ggml_backend_name(b));
        }
    }
    return b;
}
