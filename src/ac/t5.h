// Ported from sa3.cpp src/sat/t5.h (namespace sa3::sat -> ac).
// ac/t5.h -- the classic T5-base text encoder. MusicGen and MelodyFlow both condition on
// exactly this model (audiocraft T5Conditioner, name "t5-base"), and so does stable-audio-tools,
// which is why this is a straight port of sa3.cpp's sat/t5.h: learned relative position bias,
// bias-free RMS norms, ReLU feed-forward, 32128-entry vocab.
#pragma once
...
struct T5EncoderConfig {
    ...
    static T5EncoderConfig from(const GgufModel& m) {
        T5EncoderConfig c;
        c.dim                   = m.u32("ac.t5.dim");
        c.layers                = m.u32("ac.t5.layers");
        c.heads                 = m.u32("ac.t5.heads");
        c.head_dim              = m.u32("ac.t5.head_dim");
        c.intermediate          = m.u32("ac.t5.intermediate");
        c.vocab                 = m.u32("ac.t5.vocab");
        c.relative_buckets      = m.u32("ac.t5.relative_buckets");
        c.relative_max_distance = m.u32("ac.t5.relative_max_distance");
        c.eps                   = m.f32("ac.t5.eps");
        return c;
    }
};
