// Ported from sa3.cpp src/sat/tokenizer.h (namespace sa3::sat -> ac).
// ac/tokenizer.h -- compact SentencePiece-Unigram tokenizer for classic T5 (t5-base).
#pragma once
...
struct UnigramTokenizer {
    ...
    static UnigramTokenizer load(const char* path) {
        ...
        if (!raw) throw std::runtime_error("[ac tokenizer] failed to open " + std::string(path));
        std::unique_ptr<gguf_context, decltype(&gguf_free)> g(raw, gguf_free);
        auto key = [&](const char* name) {
            int i = gguf_find_key(g.get(), name);
            if (i < 0) throw std::runtime_error("[ac tokenizer] missing key " + std::string(name));
            return i;
        };
        if (std::string(gguf_get_val_str(g.get(), key("tok.model"))) != "sentencepiece-unigram")
            throw std::runtime_error("[ac tokenizer] GGUF does not contain a Unigram tokenizer");
        ...
        if (gguf_get_arr_n(g.get(), ks) != n || gguf_get_arr_type(g.get(), ks) != GGUF_TYPE_FLOAT32)
            throw std::runtime_error("[ac tokenizer] invalid token scores");
    ...
    std::vector<int32_t> encode_piece(const std::string& input) const {
        ...
        if (previous[n] < 0 && n) throw std::runtime_error("[ac tokenizer] segmentation failed");
